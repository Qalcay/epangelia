...
pub fn sha256(msg: &[u8]) -> [u8; 32] {
    let mut h = H0;
    let m1 = (msg.len() as u64).wrapping_mul(8);
    // pad: 0x80, then zeros, until len % 64 == 56, then 8 byte big endian bit length
    let mut data = msg.to_vec();
    data.push(0x80);
    while data.len() % 64 != 56 { data.push(0); }
    data.extend_from_slice(&m1.to_be_bytes());
...
pub fn hmac_sha256(key: &[u8], msg: &[u8]) -> [u8; 32] {
    ...
    let mut k = if key.len() > BLOCK { sha256(key).to_vec() } else { key.to_vec() };
    k.resize(BLOCK, 0);
